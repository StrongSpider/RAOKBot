...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('allies')
        .setDescription('Gets group allied information on player.')
        ...
        try {
            ...
            function createPage(page) {
                const replyEmbed = new EmbedBuilder()
                    .setFooter({ text: `Pages: ${page + 1}/${alliedGroupsData.length}` }) // Lets the user know what page they are viewing
                    .setTitle(userData.username + "'s Allied Connections 🔎") // Nice title, might make it something else in the future
                ...
                for (var playerGroup of alliedGroupsData[page]) {
                    // For each alliedgroup it adds a field to the embedFields array
            ...
            // Returns the command if no allied groups were found to belong in the player group list
            if (alliedGroupsData.length == 0) return await interaction.reply({ content: 'I could not find any allied groups connected to the main discord group that has a relationship with the selected user!' })

            if (alliedGroupsData.length == 1) {
                ...
                initalCollector.on('collect', async i => {
                    if (i.customId === 'prev') {
                        ...
                        var next = new ButtonBuilder()
                            ...
                            .setLabel('➡')
                            .setStyle(ButtonStyle.Secondary)
                        if (pageCount + 1 == alliedGroupsData.length) next.setDisabled(true); // If there is no pages to go next to, new button is disabled
